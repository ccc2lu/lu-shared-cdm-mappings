...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- Template to change to newspapers:
         - final level in hierarchy, i.e. the last collection without a subcollection
         - compounds with only large image children
    -->
    <!-- This is not handling any single page newspapers at this point, i.e. changing them to standalone images. -->
   ...
   <!-- make collections with one level of image children a newspaper -->
    ...
    <!-- make compounds with one level of image children a newspaper-->
    ...
    <!-- This template addresses the collection relationship of image records that are not the child of a compound.
         If the record has no non-image siblings and is to become a newspaper page, the relationship type is 
                ...
                <relatedItem xmlns="http://www.loc.gov/mods/v3" otherType="isPageOf" otherTypeAuth="dgi">
                    <identifier>
                        <xsl:call-template name="newspaper-identifier">
                            <xsl:with-param name="newspaper-identifier" select="$newspaper-identifier"/>
                        </xsl:call-template>
                    </identifier>
                </relatedItem>
            </xsl:when>
    ...
    <!-- Handling image records that are compound children requires two templates. This template tests: If 
         the record has no non-image siblings and is to become a newspaper page, the relationship type is changed 
    ...
    <xsl:template name="newspaper-identifier">
        <xsl:param name="newspaper-identifier" as="xs:string" required="yes"/>
        <xsl:choose>
            <xsl:when test="contains($newspaper-identifier,':')">
                <xsl:value-of select="substring-after($newspaper-identifier,':')"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of select="$newspaper-identifier"/>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
